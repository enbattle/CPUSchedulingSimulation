...
def check_CPU_burst(time, current_process, queue, sorted_processes_by_number, bursts, io_queue, io_times, context_switch_time, tau_dict, burst_times, alpha_value, reprocessing_queue, b_t_copy, process_termination_times):
	...
	if bursts[sorted_processes_by_number[current_process]] == 0:
		...
		process_termination_times[current_process] = time
	else:
...
def srt(some_processes, some_bursts, some_burst_times, some_io_times, context_switch_time, lambda_value, alpha_value):
	...
	for i in range(len(burst_times)):
		for j in range(len(burst_times[i])):
			tot += burst_times[i][j]
			counter1 +=1

	#print(tot, counter)
	#print(tot/counter)
	average_burst_time = tot/counter1

	sum_io_times = 0
	for i in range(len(io_times)):
		for j in range(len(io_times[i])):
			sum_io_times += io_times[i][j]
	...
	#print(some_bursts)
	process_termination_times = dict()
	#for i in range(len(processes)):
	#	process_termination_times[processes[i]] = 0
	...
	while sum(bursts) != 0:
		...
		if time == next_burst_completion and time != 0:
			...
			check_CPU_burst(time, current_process, queue, sorted_processes_by_number, bursts, io_queue, io_times, context_switch_time, tau_dict, burst_times, alpha_value, reprocessing_queue, b_t_copy2, process_termination_times)
	...
	open_file.write("-- average CPU burst time: {0:.3f} ms\r".format(average_burst_time))
